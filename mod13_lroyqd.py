...
class TestStockDataVisualizer(unittest.TestCase):
    def test_symbol(self):
        ...
        self.assertTrue(len(stockSymbol) >= 1 and len(stockSymbol) <= 7)
    ...
    def test_time_series(self):
        self.assertTrue(len(str(time_series)) == 1)
        self.assertTrue(time_series >= 1 and time_series <= 4)
    def test_start_date(self):
        self.assertEqual(f"{userDate[0]}-{userDate[1]}-{userDate[2]}", str(finalDate))
    def test_end_date(self):
        self.assertEqual(f"{endDate[0]}-{endDate[1]}-{endDate[2]}", str(finalEndDate))
